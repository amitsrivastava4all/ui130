...
document.getElementById("loadLocal").addEventListener("click",loadLocalTask);    
    }
function loadLocalTask(){
    if(localStorage.taskdata){
        var arrayObject = JSON.parse(localStorage.taskdata);
        printTask(arrayObject);
    }
}
function saveTask(){
...
console.log("Array is ",typeof taskOperations.taskList );
console.log("Array is ",taskOperations.taskList);
if(localStorage){
    localStorage.taskdata = json;
    alert("Data Save....");
}
    else{
        alert("Ur Browser is outdated not having localstorage....");
    }
}
function sort(){
